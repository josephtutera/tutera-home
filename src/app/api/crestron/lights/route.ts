...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { id, level, isOn } = body;

    if (!id) {
      return NextResponse.json(
        { success: false, error: "Missing light id" },
        { status: 400 }
      );
    }

    const client = new CrestronClient(config);
    const result = await client.setLightState({ id, level, isOn });
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to set light state" },
      { status: 500 }
    );
  }
}
